...
% FZ SETUP FOR WEAK ZONES T.Morrow JAN 11 2016
%if GEOM(2).fzSW==1
%    for bb=1:length(GEOM(2).fz)-1
%		if (GEOM(2).fzstrong(bb)~=1)
%            etan(Y>GEOM(2).top-(GEOM(2).step(bb+1)+1e3) ...
%                & Y<GEOM(2).top+GEOM(2).FZseed(2) ...
%                & X>GEOM(2).fz(bb)*max(max(X))-GEOM(2).FZseed(1) ...
%                & X<GEOM(2).fz(bb)*max(max(X))+GEOM(2).FZseed(1))=GEOM(2).weakEta;
%            etas(Y>GEOM(2).top-(GEOM(2).step(bb+1)+1e3) ...
%                & Y<GEOM(2).top+GEOM(2).FZseed(2) ...
%                & X>GEOM(2).fz(bb)*max(max(X))-GEOM(2).FZseed(1) ...
%                & X<GEOM(2).fz(bb)*max(max(X))+GEOM(2).FZseed(1))=GEOM(2).weakEta;
%        end
